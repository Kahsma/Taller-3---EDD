...
bool ArbolBinario::insertar(T value){
  ...
  NodoBinario<T>* nodoAlturas =this->raiz;
  int alturaIzquierda = this->altura(nodoAlturas->obtenerHijoIzquierdo());
  int alturaderecha = this->altura(nodoAlturas->obtenerHijoDerecho());
  int diferenciaAltura =  alturaderecha - alturaIzquierda;
  if (diferenciaAltura == -2){
    rotacionArbolDerecha();
  }
  }
  return insertado;
}
